    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < nei[v].size(); ++i) {
            ...
            if (vised.find(x) == vised.end()) {
                pathnode* next = new pathnode(x, pos[v]);//当前值以及父节点指针
                ...
                //set_intersection(sets[x].begin(), sets[x].end(), sets[v].begin(), sets[v].end(), back_inserter(next->commom));
                //set_difference(sets[x].begin(), sets[x].end(), sets[v].begin(), sets[v].end(), back_inserter(next->diff));
            }
        }
    }
    return root;
